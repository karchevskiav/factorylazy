...
export const GameState = {
  ...
  // and THREE gun turrets behind it, spaced so their ranges cover the whole wall, each
  // pre-loaded with a full magazine count.
  setupWarScreen(s) {
    const wallY = MapGen.wallRow(), W = s.map.width;
    for (let x = 0; x < W; x++)
      s.entities.push({ id: this.nextId++, type: 'stoneWall', x, y: wallY, recipe: null, modules: [] });
    const n = 3;
    for (let i = 0; i < n; i++) {
      // centre each 2-wide turret in its third of the wall (e.x+1 = centre column)
      const cx = Math.round(W * (i + 0.5) / n) - 1;
      const x = Math.max(0, Math.min(cx, W - 2));
      s.entities.push({ id: this.nextId++, type: 'gunTurret', x, y: wallY + 1, recipe: null, modules: [], ammo: WAR.turretAmmoMax });
    }
  },
  ...
  // `allowWater` lets the offshore pump sit partly on water (its own rule checks the rest).
  free(x, y, w = 2, h = 2, ignore = null, allowWater = false) {
    ...
    for (let dx = 0; dx < w; dx++) for (let dy = 0; dy < h; dy++) {
      ...
      if (!allowWater && MapGen.waterAt(this.state.map, x + dx, y + dy)) return false;  // can't build on water
    }
  ...
  // gun-turret combat bonuses from researched weapon techs (physical-projectile-damage /
  // weapon-shooting-speed). Returned as multipliers (1 = no bonus).
  weaponDamageMult() {
    let b = 0;
    for (const t of this.state.research.done) { const e = TECH[t] && TECH[t].effect; if (e && e.weaponDamage) b += e.weaponDamage; }
    return 1 + b;
  },
  weaponSpeedMult() {
    let b = 0;
    for (const t of this.state.research.done) { const e = TECH[t] && TECH[t].effect; if (e && e.weaponSpeed) b += e.weaponSpeed; }
    return 1 + b;
  },
