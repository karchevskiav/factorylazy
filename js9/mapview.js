...
export const MapView = {
  ...
  preload() {
    ...
    for (const k in ENEMIES) {
      add(ENEMIES[k].run.img); add(ENEMIES[k].attack.img);
      for (const tk in (ENEMIES[k].types || {})) { add(ENEMIES[k].types[tk].run); add(ENEMIES[k].types[tk].attack); }
    }
    add('assets/buildings/gunTurretBase.png'); add('assets/buildings/gunTurretGun.png');   // rotating-barrel turret
  },
  TURRET_GUN: { dirs: 64, cols: 8, cell: 66 },   // gunTurretGun.png atlas layout (dir 0 = North, clockwise)
  decorHash(x, y) {
  ...
  valid(type, x, y) {
    ...
    // offshore pump: straddles the shore — its footprint must be partly water, partly land
    if (def.place === 'water') {
      if (!GameState.free(x, y, w, h, null, true)) return false;   // allow water; checks bounds/obstacles/entities
      return this.waterTiles(x, y, w, h) === 1;                    // exactly one water tile (the rest is land)
    }
  ...
  // count water tiles under a footprint
  waterTiles(x, y, w, h) {
    let n = 0;
    for (let dx = 0; dx < w; dx++) for (let dy = 0; dy < h; dy++)
      if (MapGen.waterAt(GameState.state.map, x + dx, y + dy)) n++;
    return n;
  },
  ...
  render() {
    ...
    for (const e of s.entities) {
      ...
      // gun turret: static base + a 64-direction gun that points at its current target,
      // both centred on the footprint (so the barrel pivots correctly)
      if (e.type === 'gunTurret') this.drawTurret(e, sx, sy, w, h);
      else {
  ...
  // gun turret as two layers: static base ring + a gun head that faces the current
  // target (64 pre-rendered directions), both centred on the footprint.
  drawTurret(e, sx, sy, w, h) {
    const ctx = this.ctx;
    const cx = sx + w / 2, cy = sy + h / 2;          // footprint centre (screen px)
    const base = this.imgs['assets/buildings/gunTurretBase.png'];
    if (base && base.complete && base.naturalWidth) {
      const bw = w, bh = w * (base.naturalHeight / base.naturalWidth);
      ctx.drawImage(base, cx - bw / 2, cy - bh / 2, bw, bh);
    }
    // ease the gun's facing toward the target angle (shortest path); hold last aim when idle
    const TAU = Math.PI * 2, tcx = e.x + (w / TILE) / 2, tcy = e.y + (h / TILE) / 2;
    let cur = e._faceAngle || 0, target = cur;
    if (e._aim && !e._aim.gone) {
      let a = Math.atan2(e._aim.x - tcx, -(e._aim.y - tcy)); if (a < 0) a += TAU;
      target = a;
    }
    let diff = ((target - cur + Math.PI * 3) % TAU) - Math.PI;
    cur = (cur + diff * 0.35 + TAU) % TAU;
    e._faceAngle = cur;
    const G = this.TURRET_GUN, dir = ((Math.round(cur / (TAU / G.dirs)) % G.dirs) + G.dirs) % G.dirs;
    const gun = this.imgs['assets/buildings/gunTurretGun.png'];
    if (gun && gun.complete && gun.naturalWidth) {
      const C = G.cell, gsx = (dir % G.cols) * C, gsy = ((dir / G.cols) | 0) * C;
      const gw = w * 0.92, gh = gw;                  // gun head sits on the base
      ctx.drawImage(gun, gsx, gsy, C, C, cx - gw / 2, cy - gh / 2, gw, gh);
    }
  },
  ...
  drawBiters() {
    ...
    for (const b of Biters.list) {
      const anim = ENEMIES.biter[b.state]; if (!anim) continue;     // shared frame layout
      const type = ENEMIES.biter.types[b.type] || ENEMIES.biter.types.medium;
      const im = this.imgs[type[b.state]] || this.imgs[anim.img];   // per-size tinted atlas
      if (!im || !im.complete || !im.naturalWidth) continue;
      const fade = b.gone ? Math.max(0, (b.dying || 0) / 0.3) : 1;   // death fade-out
      const bw = scale * (type.scale || 1) * TILE * (b.gone ? 0.6 + 0.4 * fade : 1), bh = bw * (anim.ch / anim.cw);
