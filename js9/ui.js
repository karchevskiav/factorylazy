...
export const UI = {
  ...
  showInspector(ent) {
    ...
    if (d.place === 'ore') {
      ...
      if (ent.type === 'gunTurret') {
        const dmg = GameState.weaponDamageMult(), spd = GameState.weaponSpeedMult();
        const dps = +(WAR.turretDps * dmg * spd).toFixed(0);
        body += `<div class="insp-row dim">${I18N.t('insp_turret_combat', dps, WAR.turretRange)}</div>`;
        body += `<div class="insp-row dim">${I18N.t('insp_turret_bonus', '+' + Math.round((dmg - 1) * 100), '+' + Math.round((spd - 1) * 100))}</div>`;
      }
